...
wireguard_frpc() {
    ...
    [ -z $(grep -q "[wireguard]" "/etc/frp/frpc.ini") ] && {
...
fix_dns() {
    ...
    [ ! -z $(grep -q "#DNSStubListener=yes" "/etc/systemd/resolved.conf") ] && {
...
npm_install() {
    ...
    } || { echo "* nginxproxymanager is already running thus we cannot proceed"; exit 1; }
}
...
wgeasy_install#!/usr/bin/env bash
# Private Router (PrivateRouter.com) Reverse VPN Installer Script
# Coded by Jason Hawks <jason@fixedbit.com>
